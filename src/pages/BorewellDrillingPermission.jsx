...
import {
    Droplets, MapPin, UploadCloud, CheckCircle, ArrowRight, ArrowLeft,
    Building2, Info, Download, LayoutDashboard, LogOut, ChevronRight,
    Plus, Search, Filter, Eye, Clock, FileText
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const BorewellDrillingPermission = ({ activeCompany, setActiveCompany }) => {
    ...
    const [myApplications, setMyApplications] = useState([
        { id: 'BORE-2025-001', location: 'Jaipur, Phagi', depth: '150m', status: 'Approved', date: '2024-12-15', expiry: '2025-12-15' },
        { id: 'BORE-2025-002', location: 'Jaipur, Sanganer', depth: '200m', status: 'Pending', date: '2024-12-20', expiry: '-' }
    ]);
    ...
    const steps = [
        { id: 1, label: 'Site Details', icon: <MapPin size={18} /> },
        { id: 2, label: 'Technical Specs', icon: <Droplets size={18} /> },
        { id: 3, label: 'Documentation', icon: <FileText size={18} /> },
        { id: 4, label: 'Review & Submit', icon: <CheckCircle size={18} /> }
    ];

    const handleLogout = () => {
        if (confirm('Exit Borewell Drilling Permission? Progress may be lost.')) navigate('/dashboard');
    };

    const BorewellDashboard = () => (
        <div className="borewell-dash animated">
            ...
            <div className="applications-table">
                <div className="table-header">
                    <h3>My Borewell Drilling Permissions</h3>
                ...
                return (
                    <div className="wizard-step animated">
                        ...
                        <div className="form-layout">
                            ...
                            <div className="form-group">
                                <label>Tehsil *</label>
                                <input type="text" value={formData.tehsil} onChange={e => setFormData({ ...formData, tehsil: e.target.value })} placeholder="Enter tehsil" />
                            </div>
                            <div className="form-group full">
                                <label>Village / Area *</label>
                                <input type="text" value={formData.village} onChange={e => setFormData({ ...formData, village: e.target.value })} placeholder="Enter village or area" />
                            </div>
                            <div className="form-group full">
                                <label>Site Location Address *</label>
                                <textarea value={formData.siteLocation} onChange={e => setFormData({ ...formData, siteLocation: e.target.value })} placeholder="Complete address of the drilling site" rows="3"></textarea>
                            </div>
                        </div>
                    </div>
                );
            case 2:
                return (
                    <div className="wizard-step animated">
                        ...
                        <div className="form-layout">
                            <div className="form-group">
                                <label>Proposed Borewell Depth (meters) *</label>
                            ...
                            <div className="form-group">
                                <label>Number of Existing Borewells</label>
                                <input type="number" value={formData.existingBorewells} onChange={e => setFormData({ ...formData, existingBorewells: e.target.value })} />
                            </div>
                            <div className="form-group">
                                <label>Land Ownership Status *</label>
                                <select value={formData.landOwnership} onChange={e => setFormData({ ...formData, landOwnership: e.target.value })}>
                                    <option>Owned</option>
                    ...
                    <div className="wizard-step animated">
                        ...
                        <div className="document-checklist">
                            ...
                            <div className="doc-item">
                                ...
                                <label htmlFor="doc3">NOC from Land Owner (if leased/rented)</label>
                            </div>
                            <div className="doc-item">
                                <input type="checkbox" id="doc4" />
                                <label htmlFor="doc4">Rig Operator License</label>
                            </div>
                        </div>
                    ...
                    <div className="wizard-step animated">
                        ...
                        <div className="review-summary">
                            ...
                            <div className="summary-item">
                                ...
                                <span>{formData.village}, {formData.tehsil}, {formData.district}</span>
                            </div>
                            <div className="summary-item">
                        ...
                        <div className="consent-box">
                            ...
                            <label htmlFor="consent">I confirm that all information provided is accurate and I have obtained necessary permissions.</label>
                        </div>
                    </div>
                );
            default:
                return <div className="success-screen">
                    ...
                    <p>Your borewell drilling permission application has been submitted. Reference ID: <strong>BORE-2025-{Math.floor(Math.random() * 900 + 100)}</strong></p>
    ...
    return (
        <div className="sys-canvas">
            <aside className={`sys-sidebar ${isSidebarOpen ? 'expanded' : 'collapsed'}`}>
                <div className="sb-header">
                    <div className="sb-logo" onClick={() => navigate('/dashboard')}>
                        ...
                        {isSidebarOpen && <span>Borewell Permission</span>}
                    </div>
                ...
                <nav className="sb-nav">
                    ...
                    <button className={`sb-item ${activeMenuItem === 'dashboard' ? 'active' : ''}`} onClick={() => setActiveMenuItem('dashboard')}><Droplets size={18} /> {isSidebarOpen && "My Permissions"}</button>
                ...
                <header className="sys-header">
                    <div className="header-branding">
                        ...
                        <div className="branding-txt">
                            <h2>Borewell Drilling Permission Portal</h2>
                            <p>Prior permission required before drilling • Rajasthan Ground Water Authority</p>
                        </div>
                    </div>
            ...
            <style jsx>{`
                ...
                .logo-box { width: 40px; height: 40px; background: linear-gradient(135deg, #3b82f6, #2563eb); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 1.2rem; }
                ...
                .sb-item.active { background: linear-gradient(90deg, rgba(59, 130, 246, 0.2), transparent); color: white; border-left: 3px solid #3b82f6; }
                ...
                .ac-id { font-size: 0.7rem; color: #3b82f6; font-weight: 700; background: white; padding: 2px 6px; border-radius: 4px; }
                ...
                .help-btn:hover { background: #f8fafc; color: #3b82f6; }
                .logout-btn:hover { background: #fef2f2; border-color: #fecaca; color: #dc2626; }
                .sys-viewport { flex: 1; overflow-y: auto; background: radial-gradient(circle at 0% 0%, #f9fafb, #eff6ff 100%); padding: 40px; }
                .borewell-dash { animation: fadeIn 0.5s ease-out; }
                ...
                .btn-primary { background: #3b82f6; color: white; border: none; padding: 12px 24px; border-radius: 12px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: all 0.2s; }
                ...
                .p-node.active .node-icon { background: #3b82f6; border-color: #3b82f6; color: white; }
                .p-node span { font-size: 0.75rem; font-weight: 700; color: #64748b; }
                .p-node.active span { color: #3b82f6; }
                .p-bar { position: absolute; top: 20px; left: 0; right: 0; height: 2px; background: #e2e8f0; z-index: 0; }
                .p-bar-fill { height: 100%; background: #3b82f6; transition: width 0.3s; }
                ...
                .form-group input:focus, .form-group select:focus, .form-group textarea:focus { border-color: #3b82f6; box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.1); }
                ...
                .success-screen strong { color: #3b82f6; }
...
export default BorewellDrillingPermission;
